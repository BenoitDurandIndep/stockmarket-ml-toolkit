...
#########################################
### fork from balance module 
### without imbalance-learn due to issues with this lib
### https://github.com/scikit-learn-contrib/imbalanced-learn/issues/1081
### only the random undersampler is kept
#########################################
...
def reg_custom_undersampler_by_class(df_in: pd.DataFrame, str_label: str, nb_per_class:int,  nb_class: int = 10) -> pd.DataFrame:
    """Add a classification label to a dataframe and then undersample on the dataframe strat auto
    ...
    df_resampled=class_custom_undersampler(df_in=df_class,str_label=lab_class,nb_per_class=nb_per_class)    
...
if __name__ == "__main__":
    ...
    print(f"nb rows per quarter : {df_out['LABEL_1_class'].resample('Q').count()}")

    df_sampled=class_custom_undersampler(df_in=df_out,str_label='LABEL_1_class',nb_per_class=100)
    print(df_sampled['LABEL_1_class'].value_counts().sort_index())
    print(f"nb rows per quarter : {df_sampled['LABEL_1_class'].resample('Q').count()}")